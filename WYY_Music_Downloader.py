...
class Cracker():
	...
	def get(self, text):
		...
		print('[INFO]:Get params successfully...')
		return post_data
...
class Spider():
	...
	def get_download_url(self, songid, bit_rate=320000):
		...
		if songurl:
			print('[INFO]:Get Song {} download url...'.format(songid))
			return songurl
		else:
			print('[INFO]:Song {} is not available due to copyright issue...'.format(songid))
			return None
	# 根据歌曲下载地址下载歌曲
	def download(self, songurl, songname, songnum, save_file='./results'):
		...
		try:
			...
			print('[Warning]:Song name invalid...\nTrying to Solve the Warning...')
	...
	def post_request(self, url, params):
		...
		if res.json()['code'] != 200:
			print('[ERROR]:<post_request> error...')
		else:
			return res.json()
	# 获得cookies
...
# 下载器类
class Downloader():
	def __init__(self, timeout, cookies_path=None, save_file='./results'):
		self.spider = Spider(timeout, cookies_path)
		self.save_file = save_file
		self.timeout = timeout
	def download_song(self, songname):
		songs = None
		try:
			songs = self.spider.search_by_songname(songname)
		except:
			print('[ERROR]:<search_by_songname> error...')
		if songs != None:
			for song in songs:
				self.download_by_id(song.songid, song.songname, song.songnum, self.save_file)
				time.sleep(random.randint(1, 5))
	def download_by_id(self, songid, songname, songnum, save_file):
		try:
			...
			print('[ERROR]:<download_by_id> error...')
...
if __name__ == '__main__':
	timeout = 60
	DL = Downloader(timeout=timeout)
	print('[Tips]:Input the music you want to download...')
	song_name = input('Song Name:')
	DL.download_song(song_name)